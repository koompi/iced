...
use crate::{
    event::{self, Event}, layout::{Limits, Node}, mouse, text, scrollable, 
    Element, Hasher, Clipboard, Point, Rectangle, Length, Size, Layout, Widget,
};
   ...
   Renderer: self::Renderer,
{
   ...
   fn layout(&self, renderer: &Renderer, limits: &Limits) -> Node {
      ...
      for column in self.columns.iter() {
         let (width, height) = renderer.measure(&column.formatted_sortable_column(), text_size, self.font, Size::new(f32::INFINITY, f32::INFINITY),);
         let size = {
