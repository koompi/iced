...
use crate::{
    event::{self, Event},
    layout::{Limits, Node},
    mouse, scrollable, text, Clipboard, Element, Hasher, Layout, Length, Point,
            ...
            if i != idx {
                ...
                *order = order.toggle();
                match order {
                    ...
                    Ascending => self.data.sort_by_cached_key(|x| {
                        x.get_field_value(&name).unwrap().to_string()
                    }),
    ...
    Renderer: self::Renderer,
{
    ...
    fn layout(&self, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        let _limits = limits.width(self.width).height(Length::Shrink);
        let mut max_cols_size: Vec<Size> =
            Vec::with_capacity(self.columns.len());

        for column in self.columns.iter() {
            let (width, height) = renderer.measure(
                &column.formatted_sortable_column(),
                text_size,
                self.font,
                ...
                .for_each(|(idx, value)| {
                    ...
                    let size = {
                        let intrinsic = Size::new(
                            if let Some(max_width) = self.column_max_width {
                                width.min(max_width)
                            } else {
                                width
                            },
    ...
    fn hash_layout(&self, state: &mut Hasher) {
        ...
        self.columns
            ...
            .for_each(|((label, name), record)| {
                label.hash(state);
                format!("{}", record.get_field_value(&name).unwrap())
                    .hash(state);
            });
    }
...
impl<'a, T, Message, Renderer> From<Table<'a, T, Renderer>>
